...
func New(options Options) restgo.HTTPHandler {
	c := &Cors{
	...
	return func(ctx *restgo.Context, next restgo.Next) {
		c.HTTPHandler(ctx, next)
	}
}

// Default creates a new Cors handler with default options
func Default() restgo.HTTPHandler {
	return New(Options{})
}

// restgo compatible interface
func (c *Cors) HTTPHandler(ctx *restgo.Context, next restgo.Next) {
...
func (c *Cors) handlePreflight(ctx *restgo.Context) {
	...
	if string(ctx.Method()) != "OPTIONS" {
		c.logf("  Preflight aborted: %s!=OPTIONS", string(ctx.Method()))
		return
	}
...
func (c *Cors) handleActualRequest(ctx *restgo.Context) {
	...
	if string(ctx.Method()) == "OPTIONS" {
		c.logf("  Actual request no headers added: method == %s", string(ctx.Method()))
		return
	}
	...
	if !c.isMethodAllowed(string(ctx.Method())) {
		c.logf("  Actual request no headers added: method '%s' not allowed", string(ctx.Method()))
